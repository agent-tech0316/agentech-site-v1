...
export function AgentechGalaxyHero({
  ...
  useEffect(() => {
    ...
    if (!canvas || !hero || (!backgroundOnly && !interactionLayer)) return;
    ...
    const wordmarkMask = !backgroundOnly && titleImage ? new window.Image() : null;
    ...
    function getGalaxyMetrics() {
      ...
      const cy = h * (backgroundOnly ? 0.39 : w < 640 ? 0.31 : isDesktop ? 0.24 : 0.34);
  ...
  if (backgroundOnly) {
    return (
      <section
        ref={heroRef}
        data-agentech-galaxy-background
        aria-hidden="true"
